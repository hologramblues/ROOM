...
Two full production-shaped backend clones and two empty directories sit at the repo root, all outside the tree that CLAUDE.md documents:

```
/Users/jeremiegalan/screenflow/
  server.js                              # DEAD — 789-line "// Server V8 - Added Waitlist feature" Express+Socket.io+Mongoose backend clone. `require('./models')` and `require('./auth')` at lines 9-10 point to root-level files that do not exist (models.js / auth.js live under server/), so `node server.js` from the repo root fails immediately with MODULE_NOT_FOUND. Listens on `PORT` at line 789. Not referenced by any package.json script (root `package.json` scripts are `electron:*` only; `main: electron/main.js`). Larger AND newer than client/server.js (789 vs 627 lines, V8 vs V2). Delete — see finding 4.26 and Phase 1 step 8
  backend/                               # EMPTY — contains only `.DS_Store` and a stale `node_modules/`. No source files, no package.json, unused by any script. Delete candidate
  src/                                   # EMPTY — contains only `.DS_Store`. Unrelated to `client/src/`. Delete candidate
  package.json                           # Real root package: `main: electron/main.js`, scripts `electron:dev`/`electron:start`/`electron:build`/`postinstall`. Does NOT reference root server.js
```
...
client/
  ...
  src/
    ...
    extensions/                          # 7 files (CLAUDE.md says 5)
      ...
      remoteCursorPlugin.js              # DEAD — no importers (superseded by CollaborationCaret)
    hooks/                               # 22 files (CLAUDE.md says 20)
      # DOCUMENTED IN CLAUDE.md (20):
      useAutoSave, useDocumentLoader, useSocketConnection, useTimer,
      useTypewriterSound (INERT dead code — `setTypewriterSound` is never called anywhere in `client/src/`, so `useState(false)` never flips, the keydown listener at `useTypewriterSound.js:61` is never registered, and audio refs are `null` regardless; see §3 "Typewriter sound" row and §6 rank 10 debt),
...
| Typewriter sound | **inert dead code** | `hooks/useTypewriterSound.js:5,51,65` | Hook initializes `useState(false)` and exposes `setTypewriterSound`, but **`setTypewriterSound` is never called anywhere** (verified via `grep -rn 'setTypewriterSound' client/src/` — only the destructure at `App.jsx:196` matches, no callers), and the `typewriterSound` value is also unread. Because `typewriterSound` stays `false` forever, the effect at line 51 short-circuits with `if (!typewriterSound) return;` **before** `window.addEventListener('keydown', ...)` — so the global keydown listener is never installed and no per-keystroke cost exists. Even if the state were flipped, `audio.key/enter/backspace` are hardcoded `null` (lines 25-27), so playback is a no-op. Fully inert — safe to delete outright |
| Remote cursor plugin | **dead** | `extensions/remoteCursorPlugin.js:1` | No importers; superseded by `CollaborationCaret` |
...
|---|---|---|---|---|
| 5.1 | **`GET /api/documents/:shortId` has no ACL and auto-promotes any authenticated visitor to `editor`** | `server/server.js:485`, socket auto-add `server.js:823` | Any leaked shortId (referrer, screenshot, browser history) grants read+edit to any authenticated account. shortId is only 8 hex chars (`server.js:254`) — brute-forceable | Add `checkDocumentAccess(doc, req.user, 'viewer')`; require explicit invite for editor role; introduce a "viewer link" mode |
| 5.2 | **`POST /restore/:historyId` never verifies the entry belongs to the target doc** | `server/server.js:544-594` | Editor of doc A guesses/enumerates a `HistoryEntry._id` from doc B (MongoDB ObjectIds are time-ordered) and overwrites doc A with doc B's snapshot content — full confidentiality breach | Add `if (!entry.documentId.equals(doc._id)) return 404` before applying |
| 5.3 | **Yjs WebSocket only checks `viewer` at connect; every subsequent message is accepted** | `server/yjs-server.js:234,286-312` | A viewer/commenter (or anonymous public-viewer) can send `messageSync` frames that mutate the shared Y.Doc | Enforce role per-message; short-circuit writes for non-editors; broadcast reject signal to client |
...
| 7 | **~25** tagged `console.log` lines still in the hot editor path — 10 `[EDITOR]`/`[YJS]`/`[YJS-HOOK]` in `SingleEditor.jsx`, 6 in `useYjsProvider.js`, and 9 `[Backspace]` debug logs in `ScreenplayElement.js` (verified via `grep -n | wc -l`) | Med | Low | `components/SingleEditor.jsx` (10), `hooks/useYjsProvider.js` (6), `extensions/ScreenplayElement.js:196-241` (9) |
...
| 10 | Dead code: **root `/Users/jeremiegalan/screenflow/server.js` (789-line V8 "Waitlist" backend orphan, finding 4.26)** and **`client/server.js` (627-line legacy V2 backend, finding 4.25)** — two full production-shaped Express+Socket.io+Mongoose clones checked in outside `server/`; `remoteCursorPlugin.js`; `useTypewriterSound` (**inert** — `setTypewriterSound` never called, keydown listener never installed; not a runtime-cost item — see corrected §3 row); placeholder CSS with no attribute; unused `y-websocket` dep on server; empty `backend/` (only `node_modules`+`.DS_Store`) and `src/` (only `.DS_Store`) at repo root | Low | Low | `/Users/jeremiegalan/screenflow/server.js:1-789`, `client/server.js:1-627`, `extensions/remoteCursorPlugin.js:1`, `hooks/useTypewriterSound.js:5,51,65`, `App.css:126`, `server/package.json:24`, `backend/`, `src/` |
...
**Baseline gap (must fix first)**: this repository currently ships **zero automated tests** — verified by absence of `client/src/__tests__/`, `server/__tests__/`, and `e2e/` directories. `@testing-library/react ^16.3.2` is in `client/package.json` but no runner is configured. Phase 1 test-suite work must first bootstrap the toolchain:

- **Client unit + integration**: install and configure Vitest (or Jest via CRA's built-in support) + React Testing Library; add `test`/`test:watch` npm scripts; wire the TipTap editor test helpers.
- **E2E multi-tab**: install Playwright (`@playwright/test`), scaffold `e2e/` with a shared fixture that boots a local server + Yjs WebSocket, provide a two-tab fixture for collab specs.
- **Server API**: install `supertest`, provide a fixture that boots Express against an in-memory MongoDB (`mongodb-memory-server`) and a mock Socket.io client (`socket.io-client` in Node), so ACL specs can run without a real database.

Only after that scaffolding lands can any of the specs below run. Every "**currently fails**" note assumes the fix from §7 has NOT been applied yet — the spec is expected to fail today and pass after Phase 2–5.

**Suggested layout**: `client/src/__tests__/` for unit + integration, `e2e/` for Playwright multi-tab scenarios, `server/__tests__/` for supertest + mock socket client.
