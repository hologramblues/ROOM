...
const chatHistory = {}; // V42: Chat history storage { docId: [messages] }
...
io.on('connection', (socket) => {
  ...
  socket.on('join-document', async ({ docId }) => {
    try {
      ...
      // V42: Send chat history
      if (chatHistory[docId]) {
        socket.emit('chat-history', chatHistory[docId]);
      }
  ...
  // V42: Chat message handling
  socket.on('chat-message', ({ docId, message }) => {
    if (!docId || !message) return;
    
    // Store in history
    if (!chatHistory[docId]) chatHistory[docId] = [];
    chatHistory[docId].push(message);
    
    // Keep only last 100 messages per document
    if (chatHistory[docId].length > 100) {
      chatHistory[docId] = chatHistory[docId].slice(-100);
    }
    
    // Broadcast to other users in the room
    socket.to(docId).emit('chat-message', message);
    
    console.log('[CHAT]', message.senderName, 'in', docId, ':', message.content.substring(0, 50));
  });
